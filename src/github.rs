...
mod tests {
    use super::*;
    use crate::models::{Repository, RepositoryOwner, RepositoryLicense};
    ...
    #[test]
    fn test_github_client_new_success() {
        let client = GitHubClient::new("valid_token".to_string());
        assert!(client.is_ok());
        
        let client = client.unwrap();
        assert_eq!(client.token, "valid_token");
    ...
    #[test]
    fn test_extract_validation_error_with_message() {
        let client = GitHubClient::new("test".to_string()).unwrap();
        let error_body = json!({
        ...
        let result = client.extract_validation_error(&error_body);
        assert_eq!(result, "Validation Failed");
    }

    #[test]
    fn test_extract_validation_error_with_errors_array() {
        let client = GitHubClient::new("test".to_string()).unwrap();
        let error_body = json!({
            "errors": [
                {"message": "Invalid field 'language'"},
                {"message": "Invalid operator 'xyz'"}
            ]
        }).to_string();
        
        let result = client.extract_validation_error(&error_body);
        assert_eq!(result, "Invalid field 'language', Invalid operator 'xyz'");
    }

    #[test]
    fn test_extract_validation_error_invalid_json() {
        let client = GitHubClient::new("test".to_string()).unwrap();
    ...
    // For now, we'll test the error handling logic and validation
    ...
    async fn test_search_repositories_empty_query() {
        ...
        assert!(result.is_err());
        let error = result.unwrap_err();
        assert!(matches!(error, AppError::InvalidQuery { .. }));
        assert!(error.to_string().contains("Query cannot be empty"));
    }

    #[test]
    fn test_per_page_clamping() {
        // Test that per_page values are properly clamped to 1-100 range
        let test_cases = vec![
            (Some(0), 1),      // Below minimum
            (Some(1), 1),      // At minimum
            (Some(50), 50),    // Valid middle value
        ...
        for (input, expected) in test_cases {
            let clamped = input.unwrap_or(30).clamp(1, 100);
    ...
    fn test_page_minimum() {
        // Test that page values are properly handled
        let test_cases = vec![
            (Some(0), 1),      // Below minimum
            (Some(1), 1),      // At minimum
        ...
        for (input, expected) in test_cases {
            ...
            assert_eq!(page, expected, "Failed for input: {:?}", input);
        }
    }
}